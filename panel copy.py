...
class WebScraperPanel:
    def __init__(self,driver_path,db_path,log ):
        ...
        self.scroll_count = 3
    def get_driver(self,headless_mode):
        if not hasattr(self, '_driver'):
            self._driver = DriverManager(driver_path=self.driver_path, log=self.log,headless_mode=headless_mode)
            self.webscraper = SellerProductDataExtractor(driver=self.get_driver(headless_mode),db_handler=self.db_handler,log=log)
            self.product_extraction_scraper = ProductDetailsExtractor(db_handler=self.db_handler,driver=self.get_driver(headless_mode),log=self.log)
        return self._driver
